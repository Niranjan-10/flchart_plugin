...
class LineChartPainter extends AxisChartPainter<LineChartData> {
  ...
  List<int> prevIndicatorList = [];

  // for calculating no of clicks
  int clickHappend = 0;
  ...
  List<LineBarSpot>? handleTouch(
    ...
    // If we tap two fingers simultaneously (double finger tap)
    // it gives unpredicated  ui lines to avoid that we are handling edge cases here
    if (touchInput is PointerDownEvent) {
      clickHappend += 1;
      if (clickHappend > 1) {
        clickHappend = 0;
        prevIndicatorList.clear();
      }
    }

    // when we tap up our finger we make the previously store touch list clear
